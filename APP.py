...
def carregar_geojson():
    ...
    try:
        ...
        if not conteudo.strip():
            st.error("❌ Arquivo GeoJSON está vazio!")
            return None
        
        # Tentar fazer o parse do JSON
        try:
            geojson = json.loads(conteudo)
        except json.JSONDecodeError as e:
            st.error(f"❌ Erro ao fazer parse do GeoJSON: {e}")
            st.info("ℹ️ Verifique se o arquivo está no formato JSON válido")
            return None
        
        # Verificar se tem a estrutura correta
        if 'features' not in geojson:
            st.error("❌ GeoJSON inválido: não contém a chave 'features'")
            return None
        
        if len(geojson['features']) == 0:
            st.error("❌ GeoJSON vazio: nenhum feature encontrado")
            return None
        
        # Verificar se tem a propriedade sigla
        primeira_feature = geojson['features'][0]
        if 'properties' not in primeira_feature:
            st.error("❌ GeoJSON inválido: features não têm propriedades")
            return None
        
        if 'sigla' not in primeira_feature['properties']:
            st.warning("⚠️ GeoJSON não contém a propriedade 'sigla'. Tentando usar 'UF' ou 'name'...")
            
            # Tentar encontrar uma propriedade alternativa
            for prop in ['UF', 'name', 'uf', 'Nome', 'nome']:
                if prop in primeira_feature['properties']:
                    st.info(f"ℹ️ Usando propriedade '{prop}' como identificador")
                    # Adicionar sigla baseada na propriedade encontrada
                    for feature in geojson['features']:
                        if prop in feature['properties']:
                            feature['properties']['sigla'] = feature['properties'][prop]
                    break
            else:
                st.error("❌ GeoJSON não contém propriedade de identificação (sigla, UF ou name)")
                return None
        
        st.success(f"✅ GeoJSON carregado com sucesso! {len(geojson['features'])} estados encontrados.")
        return geojson
...
def criar_mapa_coropletico(df_mapa, geojson):
    ...
    if df_mapa.empty:
        st.warning("⚠️ Sem dados para exibir no mapa")
        return None
    
    if geojson is None:
        st.warning("⚠️ GeoJSON não disponível")
        return None
    ...
    try:
        ...
        fig.update_layout(
            ...
            title_font=dict(size=14, color=COR_AZUL_ESCURO)
        )
...
if st.session_state.df_original is not None:
    ...
    with tab_mapa:
        st.markdown("## 🗺️ Mapa de Sincronizações por Estado")
        ...
        if geojson is None:
            ...
            <div class="status-error">
                ...
                <small>Verifique se o arquivo 'estados_brasil.geojson' existe e está no formato correto.</small>
            </div>
            """, unsafe_allow_html=True)
        ...
        fig_mapa = criar_mapa_coropletico(df_mapa, geojson)
        if fig_mapa:
