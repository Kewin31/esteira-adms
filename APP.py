...
def classificar_origem_changeset(row):
    ...
    ADMS: Telemetry e Manual
    ELIPSE: ELIPSE
    """
    if 'Tipo_Chamado' not in row:
        return 'Não Classificado'
    
    tipo = str(row['Tipo_Chamado']).upper().strip()
    
    if 'TELEMETRY' in tipo:
...
def analisar_adms_elipse(df, ano_filtro=None, mes_filtro=None, empresa_filtro=None, sre_filtro=None):
    """
    Analisa os chamados comparando ADMS (Telemetry e Manual) vs ELIPSE
    Baseado no campo 'Tipo_Chamado'
    """
    ...
    df_classificados = df_analise[df_analise['Origem'] != 'Outros']
...
if st.session_state.df_original is not None:
    ...
    with tab_adms_elipse:
        ...
        st.markdown("_Análise comparativa entre chamados sincronizados do ADMS (Telemetry e Manual) e ELIPSE_")
        
        if 'Tipo_Chamado' not in df.columns:
            st.warning("⚠️ Coluna 'Tipo_Chamado' não encontrada. Não é possível realizar a análise ADMS vs ELIPSE.")
        else:
            ...
            if resultados is None:
                ...
                with col_m4:
                    ratio = resultados['total_adms'] / resultados['total_elipse'] if resultados['total_elipse'] > 0 else float('inf')
                    st.metric(
                ...
                if not resultados['empresa_origem'].empty:
                    ...
                    # Ordenar por total
                    pivot_empresa['Total'] = pivot_empresa.iloc[:, 1:].sum(axis=1)
                ...
                if not resultados['sre_origem'].empty:
                    ...
                    # Ordenar por total
                    pivot_sre['Total'] = pivot_sre.iloc[:, 1:].sum(axis=1)
                ...
                with st.expander("📋 Ver Detalhamento dos Chamados", expanded=False):
                    df_detalhe_ae = resultados['df_classificados'][['Chamado', 'Tipo_Chamado', 'Origem', 'Empresa', 'SRE', 'Status', 'Criado']].copy()
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            "Chamado": st.column_config.TextColumn("Chamado"),
                            "Tipo_Chamado": st.column_config.TextColumn("Tipo Chamado", width="medium"),
                ...
                with st.expander("📖 Entenda a Classificação ADMS vs ELIPSE"):
                    ...
                    A classificação é baseada no campo **'Tipo_Chamado'**:
                    
                    | Origem | Critérios de Classificação |
                    |--------|---------------------------|
                    | **ADMS - Telemetry** | Chamados que contêm a palavra **'TELEMETRY'** no Tipo_Chamado |
                    | **ADMS - Manual** | Chamados que contêm a palavra **'MANUAL'** no Tipo_Chamado |
                    | **ELIPSE** | Chamados que contêm a palavra **'ELIPSE'** no Tipo_Chamado |
                    | **Outros** | Chamados que não se encaixam nas categorias acima |
