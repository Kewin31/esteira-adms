...
if st.session_state.df_original is not None:
    ...
    # ============================================
# NOVA SEÇÃO: PERFORMANCE DETALHADA DOS SRES COM RETORNO CLIENTE
# (Adicionar após a tabela "Performance Detalhada dos SREs" existente)
# ============================================

# Separador visual
st.markdown("---")
st.markdown('<div class="section-title-exec">📊 PERFORMANCE DETALHADA DOS SRES - MÉTRICA SRE</div>', unsafe_allow_html=True)

# Container expansível com explicação do cálculo
with st.expander("📘 **Como é calculada a Performance do SRE?**", expanded=True):
    st.markdown("""
    ### 🧮 Fórmula de Performance do SRE

    A performance é calculada usando a seguinte fórmula:

    ```
    Performance = (Cards Sincronizados - Cards Retorno Cliente) / 
                   (Total Cards + ABS(((Total_Geral_Cards / Total_Cards_SRE) / Total_SREs) - 1))
    ```

    **📌 Onde:**
    - **Cards Sincronizados**: Total de cards que o SRE sincronizou
    - **Cards Retorno Cliente**: Cards que voltaram do cliente para correção (qualquer pessoa pode modificar)
    - **Total Cards**: Total de cards que passaram pelo SRE
    - **Total_Geral_Cards**: Total de cards na esteira (A2)
    - **Total_SREs**: Número total de SREs (B2)

    **🎯 O que a fórmula mede:**
    - **Numerador (C5-D5)** = Acertos líquidos (sincronizados que não voltaram)
    - **Denominador** = Total de cards + fator de ajuste por distribuição de carga
    
    O fator de ajuste considera se o SRE está acima ou abaixo da média de distribuição de cards.
    """)

# Verificar se temos os dados necessários
if 'SRE' in df_sre.columns and 'Status' in df_sre.columns and 'Revisões' in df_sre.columns:
    
    # ============================================
    # CÁLCULO DOS INDICADORES PARA CADA SRE
    # ============================================
    
    # Obter lista única de SREs (usando a função de substituição de nomes)
    sres_list = df_sre['SRE'].dropna().unique()
    
    # Valores globais para o cálculo
    total_geral_cards = len(df_sre)  # A2 - Total de cards na esteira
    total_sres = len(sres_list)      # B2 - Total de SREs
    
    # Lista para armazenar os dados de performance
    performance_data = []
    ...
    for sre in sres_list:
        ...
        if len(df_sre_data) > 0:
            ...
            # RETORNO CLIENTE: Cards que voltaram (revisões > 0)
            if 'Revisões' in df_sre_data.columns:
                cards_retorno_cliente = len(df_sre_data[df_sre_data['Revisões'] > 0])
            else:
                cards_retorno_cliente = 0
            
            # CÁLCULO DA PERFORMANCE SEGUNDO A FÓRMULA ORIGINAL
            numerador = sincronizados - cards_retorno_cliente
            
            # Calcular o denominador com ajuste
            if total_cards > 0 and total_sres > 0:
                fator_ajuste = (total_geral_cards / total_cards) / total_sres
                fator_abs = abs(fator_ajuste - 1)
                denominador = total_cards + fator_abs
                
                if denominador > 0:
                    performance = (numerador / denominador) * 100
                else:
                    performance = 0
            else:
                performance = 0
            ...
            # Nome formatado do SRE
            nome_sre_display = substituir_nome_sre(sre)
            
            performance_data.append({
                'SRE': nome_sre_display,
                'Total Cards': total_cards,
                'Sincronizados (C)': sincronizados,
                'Retorno Cliente (D)': cards_retorno_cliente,
                'Acertos Líquidos (C-D)': numerador,
                'Fator Ajuste': round(fator_ajuste, 3),
    ...
    if performance_data:
        ...
        # ============================================
        # GRÁFICO DE BARRAS DA PERFORMANCE
        # ============================================
        st.markdown("### 📈 Performance dos SREs")
        
        fig_perf = go.Figure()
        
        # Definir cores baseadas no valor
        max_perf = df_performance['Performance (%)'].max()
        min_perf = df_performance['Performance (%)'].min()
        
        colors = []
        for perf in df_performance['Performance (%)']:
            if max_perf == min_perf:
                colors.append('#28a745')
            else:
                normalized = (perf - min_perf) / (max_perf - min_perf)
                red = int(220 * (1 - normalized) + 40 * normalized)
                green = int(53 * (1 - normalized) + 167 * normalized)
                blue = int(69 * (1 - normalized) + 69 * normalized)
                colors.append(f'rgb({red}, {green}, {blue})')
        
        fig_perf.add_trace(go.Bar(
            x=df_performance['SRE'],
            y=df_performance['Performance (%)'],
            text=df_performance['Performance (%)'].apply(lambda x: f'{x:.1f}%'),
            textposition='outside',
            marker_color=colors,
            marker_line_color='#0c2461',
            marker_line_width=1.5,
        ...
        # GRÁFICO DE RADAR
        # ============================================
        st.markdown("### 📊 Análise Radar de Performance")
        
        col_radar1, col_radar2 = st.columns([3, 1])
        
        with col_radar1:
            # Preparar dados para o gráfico de radar
            radar_data = df_performance.copy()
            
            # Normalizar métricas para escala 0-100
            max_cards = radar_data['Total Cards'].max()
            radar_data['Total Cards (Norm)'] = (radar_data['Total Cards'] / max_cards * 100).round(1) if max_cards > 0 else 0
            
            max_sinc = radar_data['Sincronizados (C)'].max()
            radar_data['Sincronizados (Norm)'] = (radar_data['Sincronizados (C)'] / max_sinc * 100).round(1) if max_sinc > 0 else 0
            
            max_retorno = radar_data['Retorno Cliente (D)'].max()
            if max_retorno > 0:
                radar_data['Qualidade (Inv. Retorno)'] = 100 - (radar_data['Retorno Cliente (D)'] / max_retorno * 100)
            else:
                radar_data['Qualidade (Inv. Retorno)'] = 100
            
            max_acertos = radar_data['Acertos Líquidos (C-D)'].max()
            radar_data['Acertos (Norm)'] = (radar_data['Acertos Líquidos (C-D)'] / max_acertos * 100).round(1) if max_acertos > 0 else 0
            
            # Criar gráfico de radar
            fig_radar = go.Figure()
            
            categorias = ['Total Cards', 'Sincronizados', 'Qualidade', 'Acertos']
            
            for idx, row in radar_data.iterrows():
                valores = [
                    row['Total Cards (Norm)'],
                    row['Sincronizados (Norm)'],
                    row['Qualidade (Inv. Retorno)'],
                    row['Acertos (Norm)']
                ]
                valores += valores[:1]
                ...
                fig_radar.add_trace(go.Scatterpolar(
                    ...
                    fill='toself',
                    name=f"{row['SRE']} ({row['Performance (%)']:.1f}%)",
                    line_color=cor,
                    opacity=0.7
                ))
        ...
        with col_radar2:
            st.markdown("""
            <div style="background: #f8f9fa; padding: 1rem; border-radius: 10px; border-left: 4px solid #1e3799;">
                <h4 style="margin: 0 0 1rem 0;">📊 Categorias</h4>
                <p><strong>Total Cards:</strong> Volume de trabalho</p>
                <p><strong>Sincronizados:</strong> Capacidade de finalização</p>
                <p><strong>Qualidade:</strong> Inverso do retorno</p>
                <p><strong>Acertos:</strong> Sincronizados sem retorno</p>
                <hr>
                <p><span style="color: #28a745;">🟢 Verde:</span> ≥ 95%</p>
                <p><span style="color: #007bff;">🔵 Azul:</span> ≥ 85%</p>
                <p><span style="color: #ffc107;">🟡 Amarelo:</span> ≥ 70%</p>
                <p><span style="color: #dc3545;">🔴 Vermelho:</span> < 70%</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        display_performance = df_performance[[
            'SRE', 'Total Cards', 'Sincronizados (C)', 'Retorno Cliente (D)',
            'Acertos Líquidos (C-D)', 'Performance (%)'
        ]].copy()
        
        def get_status(perf):
            if perf >= 95: return "⭐ Excelente"
            elif perf >= 85: return "✅ Bom"
            elif perf >= 70: return "⚠️ Regular"
            else: return "🔴 Atenção"
        
        display_performance['Status'] = display_performance['Performance (%)'].apply(get_status)
        ...
        st.dataframe(
            ...
            column_config={
                "SRE": st.column_config.TextColumn("SRE"),
                "Total Cards": st.column_config.NumberColumn("Total", format="%d"),
                "Sincronizados (C)": st.column_config.NumberColumn("Sinc.", format="%d"),
                "Retorno Cliente (D)": st.column_config.NumberColumn("Retorno", format="%d"),
                "Acertos Líquidos (C-D)": st.column_config.NumberColumn("Acertos", format="%d"),
        ...
        # BOTÃO DE EXPORTAÇÃO
        # ============================================
        csv_performance = display_performance.to_csv(index=False).encode('utf-8-sig')
        st.download_button(
            label="📥 Exportar Performance SRE (CSV)",
            data=csv_performance,
            file_name=f"performance_sre_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv",
            use_container_width=True
        )
    # ============================================
