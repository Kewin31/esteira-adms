...
def calcular_hash_arquivo_por_caminho(caminho):
    """Calcula hash de um arquivo pelo caminho"""
    try:
...
def inicializar_sessao():
    """Inicializa todas as variáveis da sessão"""
    if 'inicializado' not in st.session_state:
        ...
        st.session_state.caminho_arquivo_local = None
        st.session_state.modo_carregamento = None  # 'upload' ou 'local'

def carregar_arquivo_local():
    """Carrega automaticamente arquivo local se existir"""
    caminho_encontrado = encontrar_arquivo_dados()
    
    if caminho_encontrado:
        # Verificar se é um arquivo diferente do atual
        current_hash = calcular_hash_arquivo_por_caminho(caminho_encontrado)
        
        if (st.session_state.caminho_arquivo_local != caminho_encontrado or 
            ...
            with st.spinner('📂 Carregando dados locais...'):
                ...
                if df_local is not None:
                    ...
                    st.session_state.caminho_arquivo_local = caminho_encontrado
                    st.session_state.ultima_atualizacao = get_horario_brasilia()
                    st.session_state.modo_carregamento = 'local'
        ...
        if ('file_hash' not in st.session_state or 
            ...
            with st.spinner('🔄 Processando novo arquivo...'):
                ...
                if df_novo is not None:
                    ...
                    st.session_state.ultima_atualizacao = get_horario_brasilia()
                    st.session_state.caminho_arquivo_local = None  # Resetar caminho local
                    st.session_state.modo_carregamento = 'upload'
                    
                    # Limpar cache
                    st.cache_data.clear()
                    
                    st.success(f"✅ {len(df_novo):,} registros carregados via upload!")
                    return True
                else:
                    st.error(f"❌ {status}")
                    return False
...
def verificar_atualizacao_simples():
    """Verificação simplificada de atualização de arquivo local"""
    if (st.session_state.modo_carregamento == 'local' and 
        st.session_state.caminho_arquivo_local and 
        os.path.exists(st.session_state.caminho_arquivo_local)):
        
        current_hash = calcular_hash_arquivo_por_caminho(st.session_state.caminho_arquivo_local)
        
        if current_hash and current_hash != st.session_state.file_hash:
...
def limpar_sessao_dados():
    ...
    keys_to_clear = [
        ...
        'caminho_arquivo_local', 'modo_carregamento', 'carregamento_inicial',
        'inicializado'
    ]
...
# ============================================
# SIDEBAR - FILTROS E CONTROLES (OTIMIZADO)
# ============================================
with st.sidebar:
    # Logo e título
    st.markdown("""
    ...
    # 1. INICIALIZAÇÃO (sempre executado)
    inicializar_sessao()
    
    # 2. CARREGAMENTO AUTOMÁTICO INICIAL (executado apenas uma vez)
    if (st.session_state.df_original is None and 
        'carregamento_inicial' not in st.session_state):
        
        st.session_state.carregamento_inicial = True
        
        # Tentar carregar arquivo local automaticamente
        if carregar_arquivo_local():
            st.rerun()
    
    # 3. VERIFICAÇÃO DE ATUALIZAÇÃO (sempre verificado)
    if verificar_atualizacao_simples():
        st.warning("⚠️ O arquivo local foi modificado!")
        
        if st.button("🔄 Atualizar agora", use_container_width=True, key="btn_atualizar_agora"):
            if carregar_arquivo_local():
                st.rerun()
    
    # 4. SE HOUVER DADOS CARREGADOS - MOSTRAR FILTROS
    ...
    # 5. CONTROLES DE ARQUIVO (SEMPRE VISÍVEL)
    with st.container():
        st.markdown('<div class="sidebar-section">', unsafe_allow_html=True)
        st.markdown("**📁 Gerenciamento de Arquivos**")
        
        # Status atual
        if st.session_state.df_original is not None:
            modo = st.session_state.modo_carregamento
            nome_arquivo = os.path.basename(st.session_state.arquivo_atual) if st.session_state.arquivo_atual else "Desconhecido"
            
            st.markdown(f"""
            <div style="background: #f8f9fa; padding: 0.8rem; border-radius: 8px; margin-bottom: 1rem;">
                <p style="margin: 0 0 0.3rem 0; font-weight: 600;">
                {'📤' if modo == 'upload' else '💾'} Arquivo atual:
                </p>
                <p style="margin: 0; font-size: 0.9rem; color: #495057;">{nome_arquivo}</p>
                <p style="margin: 0.3rem 0 0 0; font-size: 0.8rem; color: #6c757d;">
                📊 {len(st.session_state.df_original):,} registros | 
                🕐 {st.session_state.ultima_atualizacao or 'N/A'}
                </p>
            </div>
            """, unsafe_allow_html=True)
            ...
            with col_btn1:
                if st.button("🔄 Recarregar", 
                    ...
                    if st.session_state.modo_carregamento == 'local' and st.session_state.caminho_arquivo_local:
                        if carregar_arquivo_local():
                            st.rerun()
                    elif st.session_state.modo_carregamento == 'upload':
                        st.info("ℹ️ Para recarregar arquivo enviado, faça upload novamente")
            
            with col_btn2:
                if st.button("🗑️ Limpar Tudo", 
                           use_container_width=True,
                           type="secondary",
                           help="Limpa todos os dados",
                           key="btn_limpar"):
        ...
        st.markdown("**📤 Importar Novo Arquivo**")
        ...
        if uploaded_file is not None:
            if processar_upload(uploaded_file):
                st.rerun()
        ...
        if st.button("💾 Carregar arquivo local", 
                    use_container_width=True,
                    type="secondary",
                    help="Tenta carregar arquivo local automaticamente"):
            
            if carregar_arquivo_local():
                st.rerun()
            else:
                st.error("❌ Nenhum arquivo local encontrado!")
